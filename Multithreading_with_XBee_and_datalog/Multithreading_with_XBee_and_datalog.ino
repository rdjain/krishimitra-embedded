...
//SoftwareSerial nodeSerial(6,7);!!!!!CHANGE!!!!!
...
String msg = "" ; 
String message(){
  String ("Your field temperature is " String(Temperature())
  }
void pollGSM(){
    long int start;
    if(count == 0){
...
void xbeeMotorControl(int access, int val){
    Serial.println("testapp access : "+String(access));
    if(access < 4 && val == 1){  // JIO
        zigSerial.write(Router_on1, 20);
      } 
      else if(access < 4 && val == 2){ //JIO
 ...
 void sendSMS(int access, int val){
      ...
      delay(100);
      mySerial.println(msg);// The SMS te want to send
      delay(100);
       mySerial.println((char)26);// ASCII code of CTRL+Z
      delay(250);
  }
////int conductivity(int user){
//  
